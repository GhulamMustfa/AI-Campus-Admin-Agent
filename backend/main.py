...
async def chat(msg: ChatMessage):
    try:
        response, tools_used = await run_agent_with_tools(msg.message, msg.user_id, use_memory=False)
...
async def chat_stream(msg: ChatMessage):
    async def event_stream():
        try:
            response, _ = await run_agent_with_tools(msg.message, msg.user_id, use_memory=True)
